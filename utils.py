...
def make_cosine_acc_scheduler(
    optimizer,
    total_iters: int,
    eta_min: float = 2e-5,
    acc_threshold: float = 10.0,
    drop_factor: float = 0.5,
    cooldown_epochs: int = 2,
):
    """
    Create a cosine annealing scheduler with accuracy-triggered LR drops.
    - Use scheduler.step() as usual (e.g., per-iteration).
    - Call on_epoch_end(epoch_idx, epoch_accuracy) at the end of each epoch.
      If epoch_accuracy < acc_threshold and at least `cooldown_epochs` have passed
      since the last drop, the LR (and base_lrs) are scaled by `drop_factor`.

    Returns:
      scheduler, on_epoch_end (callable)
    """
    scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(
        optimizer, T_max=total_iters, eta_min=eta_min
    )
    state = {"last_drop_epoch": -10**9}

    def _scale_scheduler_lr(sched, optim, factor: float, floor_lr: float):
        for group in optim.param_groups:
            group["lr"] = max(group["lr"] * factor, floor_lr)
        if hasattr(sched, "base_lrs"):
            sched.base_lrs = [max(lr * factor, floor_lr) for lr in sched.base_lrs]

    def on_epoch_end(epoch_idx: int, epoch_accuracy: float):
        if epoch_accuracy < acc_threshold and (epoch_idx - state["last_drop_epoch"]) >= cooldown_epochs:
            _scale_scheduler_lr(scheduler, optimizer, drop_factor, eta_min)
            state["last_drop_epoch"] = epoch_idx

    return scheduler, on_epoch_end
