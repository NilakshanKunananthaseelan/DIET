"""
Euclidean Space Version of LoRA-based Semantic Unlearning

This script converts the hyperbolic space implementation to use only Euclidean space.
Key changes:
- Removed geoopt dependency and hyperbolic manifold setup
- Prototypes are generated and used in Euclidean space
- Distance calculations use standard Euclidean metrics
- LoRA training adapted for Euclidean geometry
"""
...
from lora_euclidean import run_lora_euclidean
import clip
...
def setup_deterministic_seed(seed):
    """Set up all random number generators with the same seed"""
    # Set numpy seed
    np.random.seed(seed)
...
def get_consistent_rng(seed):
    """Get a consistent random number generator"""
...
def setup_coop_dataloaders(args, preprocess=None):
    ...
    """
    seed = args.seed
    # Set up deterministic seed at the beginning
    setup_deterministic_seed(seed)
    
    # Use consistent RNG throughout
    rng = get_consistent_rng(seed) 
    ...
    forget_val_indices = []
    for idx, (x, y) in enumerate(val_loader.dataset):
        if y == args.class_to_replace:
            forget_val_indices.append(idx)

    if len(forget_val_indices) > 8:
        forget_val_indices = random.sample(forget_val_indices, 8)

    forget_val_loader = torch.utils.data.DataLoader(
        Subset(val_loader.dataset, forget_val_indices),
        batch_size=args.batch_size,
        shuffle=False,
        num_workers=args.workers,
    )
...
def main():
    ...
    forget_val_loader = unlearn_data_loaders['forget_val']

    # Set up Euclidean space for prototypes (no manifold needed)
    args.manifold = None  # No manifold for Euclidean space
    clip_model.cuda()
    ...
    proto_type = getattr(args, "prototype_type", "eucl")
    args.proto_type = proto_type

    # Generate prototypes according to the specified type
    if proto_type == "random":
        # Generate random prototypes in Euclidean space
        proto_h = torch.randn(len(proto_classes), 512).cuda()
        proto_h = proto_h / proto_h.norm(dim=-1, keepdim=True) * args.norm_r
    elif proto_type == 'eucl':
        proto = proto_embeddings
        # Normalize prototypes to specified radius
        proto_h = norm_clip(proto, r=args.norm_r)
    else:
        raise ValueError(f"Unknown prototype_type: {proto_type}")

    
    
    # Run LoRA-based unlearning with Euclidean space
    clip_model = run_lora_euclidean(
        args,
        clip_model,
        logit_scale,
        classnames,
    ...
    if "new_accuracy" not in evaluation_result:
        ...
        for name, loader in unlearn_data_loaders.items():
            val_acc = evaluate_lora(args, clip_model, loader, template, classnames)
