...
def apply_lora_ffn(args, clip_model):
    list_lora_layers = []
    if args.encoder == 'text' or args.encoder == 'both':
        indices = INDEX_POSITIONS_TEXT[args.position]
        text_encoder = clip_model.transformer
        for i, block in enumerate(text_encoder.resblocks):
            ...
            if i in indices:
                # print(f"Residual Attention Block {i}: {block}")
                for name, submodule in block.named_children():
                    # print(name,submodule.__class__)
                    if isinstance(submodule, nn.Sequential) or isinstance(submodule, nn.modules.container.Sequential):
                       
            
                        submodule=nn.Sequential(OrderedDict([
                        ("c_fc", LinearLoRA(submodule.c_fc, r=args.r, lora_alpha=args.alpha, dropout_rate=args.dropout_rate)),
                        ("gelu", QuickGELU()),
                        ("c_proj", LinearLoRA(submodule.c_proj, r=args.r, lora_alpha=args.alpha, dropout_rate=args.dropout_rate))]
                        ))
                        
                        
                        setattr(block, name, submodule)
                        list_lora_layers.append(submodule)
...
def save_lora_ffn(args, list_lora_layers):
    weights = {}
    for i, layer in enumerate(list_lora_layers):
        layer_weights = {}
        # layer_weights['c_fc'] = {'w_lora_A':layer.c_fc.w_lora_A.data,
                            #  'w_lora_B':layer.c_fc.w_lora_B.data}
        layer_weights['c_proj'] = {'w_lora_A':layer.c_proj.w_lora_A.data,
                                'w_lora_B':layer.c_proj.w_lora_B.data}
...
def load_lora(args, list_lora_layers):
    ...
    if args.eval_only:
        load_path = f'{args.save_path}'
        print(f'Eval only: Loading from {load_path}')
...
def load_lora_ffn(args, list_lora_layers):
    # to manage names like ViT-B/16
    backbone = args.backbone.replace('/', '').replace('-', '').lower()
    load_path = f'{args.save_path}'
    if not os.path.exists(load_path):
    ...
    for i, layer in enumerate(list_lora_layers):
        layer_weights = weights[f'layer_{i}']
        # if 'c_fc' in layer_weights:
        #     # print(layer.c_fc.w_lora_A.data.shape,layer_weights['c_fc']['w_lora_A'].shape)
        #     layer.c_fc.w_lora_A.data.copy_(
        #         layer_weights['c_fc']['w_lora_A'])
        #     layer.c_fc.w_lora_B.data.copy_(
        #         layer_weights['c_fc']['w_lora_B'])
        if 'c_proj' in layer_weights:
            layer.c_proj.w_lora_A.data.copy_(layer_weights['c_proj']['w_lora_A'])
            layer.c_proj.w_lora_B.data.copy_(layer_weights['c_proj']['w_lora_B'])
