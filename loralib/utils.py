...
def load_lora__(args, list_lora_layers):
    # to manage names like ViT-B/16
    backbone = args.backbone.replace('/', '').replace('-', '').lower()
    load_path = f'{args.save_path}/{args.seed}/{backbone}/{args.dataset}/{args.shots}shots/seed{args.seed}/{args.filename}.pt'
...
def load_lora(args, list_lora_layers):
    ...
    loaded_data = torch.load(load_path,map_location=args.device)
    ...
    for i, layer in enumerate(list_lora_layers):
        layer_weights = weights[f'layer_{i}']
        if 'q' in args.params and 'q_proj' in layer_weights:
            # print(layer.q_proj.w_lora_A.data.mean(), layer_weights['q_proj']['w_lora_A'].mean())
            layer.q_proj.w_lora_A.data = layer_weights['q_proj']['w_lora_A']
            # print(layer.q_proj.w_lora_A.data.mean())
            layer.q_proj.w_lora_B.data = layer_weights['q_proj']['w_lora_B'].clone()
        if 'k' in args.params and 'k_proj' in layer_weights:
            layer.k_proj.w_lora_A.data = layer_weights['k_proj']['w_lora_A'].clone()
            layer.k_proj.w_lora_B.data = layer_weights['k_proj']['w_lora_B'].clone()
        if 'v' in args.params and 'v_proj' in layer_weights:
            layer.v_proj.w_lora_A.data = layer_weights['v_proj']['w_lora_A'].clone()
            layer.v_proj.w_lora_B.data = layer_weights['v_proj']['w_lora_B'].clone()
        if 'o' in args.params and 'proj' in layer_weights:
            layer.proj.w_lora_A.data = layer_weights['proj']['w_lora_A'].clone()
            layer.proj.w_lora_B.data = layer_weights['proj']['w_lora_B'].clone()
    print(f'LoRA weights loaded from {load_path}')
